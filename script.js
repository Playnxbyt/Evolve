  ...
  list.innerHTML=CATS.map(cat=>{
    ...
    const goalsHTML=catGoals.length===0
      ...
      :catGoals.map((g,i)=>{
        ...
        return `<div class="goal-row" data-cat="${cat.id}" data-gid="${g.id}" onclick="toggleGoal('${cat.id}','${g.id}')" style="background:${isDone?cat.color+"20":"rgba(255,255,255,0.04)"};border-color:${isDone?cat.color+"60":"rgba(255,255,255,0.07)"}">
...
function toggleTaskCat(catId) {
  ...
  if (card) {
    ...
    card.style.transform  = "scale(0.977)";
    setTimeout(() => {
  ...
  if (isOpen) {
    ...
    // Quick fade-out rows (GPU only)
    panel.querySelectorAll(".task-goal-row").forEach(row => {
      row.style.transition = "opacity 0.12s ease, transform 0.15s ease";
      row.style.opacity    = "0";
      row.style.transform  = "translateY(-4px)";
    });

    // Lock pixel height → animate height + scaleY fold feel
    const lockedH = panel.offsetHeight;
    ...
    // ════ UNFOLD / EXPAND ══════════════════════
    // Reset rows to invisible start position
    panel.querySelectorAll(".task-goal-row").forEach(row => {
      row.style.cssText = "opacity:0;transform:translateY(10px)";
    });

    // Lay panel out invisibly to measure true height
    panel.style.cssText = [
      "display:flex",
    ...
    // Measure after single rAF so browser has laid out
    requestAnimationFrame(() => {
      const fullH = panel.scrollHeight;

      // Start unfold: grow height + fade in, with slight scaleY from top
      panel.style.transition = [
        "height 0.44s cubic-bezier(0.34,1.05,0.64,1)",
        "opacity 0.3s ease",
      ].join(",");
      ...
      // Stagger rows as card unfolds — waterfall feel
      panel.querySelectorAll(".task-goal-row").forEach((row, i) => {
        setTimeout(() => {
          row.style.transition = "opacity 0.28s ease, transform 0.34s cubic-bezier(0.34,1.5,0.64,1)";
          row.style.opacity    = "1";
          row.style.transform  = "translateY(0)";
        }, 80 + i * 60);
      });

      // After open: free height so dynamic content works
      panel.addEventListener("transitionend", () => {
...
function renderCalendar() {
  ...
  for(let d=1;d<=days;d++){
    ...
    html+=`<div class="cal-day${isToday?' today':''}" style="background:${bg};border:${border};${shadow};opacity:0;transform:scale(0.88)" onclick="showDayGoals(${y},${m},${d})">
  ...
  // GPU-only stagger: opacity+transform only, no layout triggers
  requestAnimationFrame(()=>{
    const cells=grid.querySelectorAll(".cal-day");
    cells.forEach((el,i)=>{
      const delay=i*14; // ms between cells
      setTimeout(()=>{
        el.style.transition="opacity 0.28s ease, transform 0.32s cubic-bezier(0.34,1.2,0.64,1)";
        el.style.opacity="1";
          ...
          return `<div class="task-month-cat-card" style="border-color:${cat.color}22;background:${cat.bg}">
            <div class="task-month-cat-header" onclick="toggleTaskCat('${cat.id}')">
              ...
              <div style="display:flex;align-items:center;gap:8px">
                ...
                <span class="task-month-chevron">›</span>
              </div>
            </div>
